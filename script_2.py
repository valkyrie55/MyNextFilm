    ...
    if line.startswith('INT') or line.startswith('EXT') or line.startswith('EXT/INT') or line.startswith('INT/EXT'):
        ...
        if word.split('(')[0].strip() in characters:
            ...
            if not (len(dia)==0 and parenthetical=="NONE"):
                ...
                [2] = dialogue
                """
                # list_speaker.append(scene_number)
                # list_speaker.append(speaker)
                # list_speaker.append(parenthetical)
                # list_speaker.append(dia)

                # #mydic2[speaker] = [scene_number, parenthetical, dia]  #added 28/May/2020
                # print("My dictionary: ", mydic2)
...
#print("SCene dictionary: ", scene_dic)
#print("My dictionary: ", mydic2)
# print("Speakers information: ",  speakers_info)
...
# #parenthetical
#     p_data = " ".join(dialogue_parenthetical)
#     p = doc.add_paragraph(p_data)
#     p_format = p.paragraph_format
#     p_format.keep_with_next = True
#     p_format.space_after = 0
#     p_format.line_spacing = Pt(12)
#     p_format.left_indent = Inches(2)
#     p_format.right_indent = Inches(1.25)

# #dialogue
#     dialogue = " ".join(dialogue_delivery)
#     d = doc.add_paragraph()
#     d.add_run(dialogue)
#     d_format = d.paragraph_format
#     d_format.keep_together = True
...
for s in range(2, scene_no+1):
    ...
    for w in words:
        if w['scene_num'] == s:
            if w['type'] == 'SL':                     #SLUGLINE
            ...
            elif w['type'] == 'DL_SPEAKER':   #its a speaker  
                ...
                if w['zero_one'] == '0':   #to be removed
                    ...
                    for t in w['word']:
                        res = res + t +  '\u0336'   #strike through
                    dialogue_parenthetical.append(res)
                else:
                    dialogue_parenthetical.append(w['word'])

    create_script(doc ,dialogue_speaker, dialogue_delivery, dialogue_parenthetical, slugline_list, action_list)
    # print("---------------------------------------------")    

print("Done!")
# print("Speakers in the scene: ", dialogue_speaker)
# print("Dialogues in the scene: ", dialogue_delivery)